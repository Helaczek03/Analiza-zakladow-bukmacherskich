...
with
source as (
    select * from {{ source('raw', 'stare_zdarzenia') }}
),

final as (
    select
        concat(ticket_number, '_', row_number() over (partition by ticket_number order by ticket_number)) as id_zdarzenia,
        cast(ticket_number as bigint) as id_kuponu,
        opportunity as mecz,
        cast(id_event as bigint) as id_rozgrywek,
        ...
        case try_cast(odds_is_winning as numeric(2,1))
            when 1 then 'tak'
            when 0 then 'nie'
            else 'anulowane'
    ...
    and try_cast(id_event as bigint) is not null
